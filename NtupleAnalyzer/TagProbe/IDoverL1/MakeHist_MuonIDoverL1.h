...
public:
  ...
  {
    Bool_t flag = kFALSE;
    if( dRMatching_IterL3MuonNoID(probe_.vecP, ntuple_, -1, 0.1, IDType_) )
      flag = kTRUE;
...
public:
  ...
  void Set_IDType( TString IDType ) { IDType_ = IDType; }

  void Produce()
  {
    if( IDType_ == "" )
    {
      cout << "ID type is not set yet!" << endl;
      return;
    }
...
void Test()
{
  ...
  histProducer->AddDataPath("/Users/KyeongPil_Lee/ServiceWorks/MuonHLTTool/v04_MuonHLTID/MuonHLTNtupler/test/ntuple.root");
