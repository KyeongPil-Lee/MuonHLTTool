...
public:
  ...
  TnPPairBase(NtupleHandle* ntuple)
  {
    ntuple_ = ntuple;
  ...
  // -- when you set tag and probe later
  void Set_Tag( MuonHLT::Muon tagCand )     { tag_ = tagCand; }
  void Set_Probe( MuonHLT::Muon probeCand ) { probe_ = probeCand; }
  ...
  virtual Bool_t IsTag(MuonHLT::Muon tagCand, MuonHLT::NtupleHandle* ntuple) {
    Bool_t flag = kFALSE;

    cout << "default IsTag(tagCand, ntuple)" << endl;

    return flag;
  }

  // -- user-defined probe condition
  virtual Bool_t IsProbe(MuonHLT::Muon probeCand, MuonHLT::NtupleHandle* ntuple) {
    Bool_t flag = kFALSE;

    cout << "default IsProbe(probeCand, ntuple)" << endl;

    return flag;
  }

  // -- user-defined passing probe condition
  virtual Bool_t IsPassingProbe(MuonHLT::Muon probeCand, MuonHLT::NtupleHandle* ntuple) {
    Bool_t flag = kFALSE;

    cout << "default IsPassingProbe(probeCand, ntuple)" << endl;

    return flag;
  }

  // -- could be re-defined
  virtual Bool_t IsGoodPair(MuonHLT::Muon tagCand, MuonHLT::Muon probeCand, MuonHLT::NtupleHandle* ntuple)
  {
    Bool_t flag = kFALSE;
    ...
    if( 49.9 < mass && mass < 130.1 && deltaR > 0.3 )
      flag = kTRUE;
...
public:
  ...
  TnPHist( TString tag, TString binType, Double_t minPtCut, vector<Double_t> vec_BinEdge )
  {
    tag_      = tag;
    binType_  = binType;
    minPtCut_ = minPtCut;
...
private:
  ...
  void GenerateHists()
  {
    ...
    for(Int_t i=0; i<nBin_; i++)
    {
      ...
      if( tag_ != "" ) histNamePass = histNamePass + "_" + tag_;
      ...
      if( tag_ != "" ) histNameFail = histNameFail + "_" + tag_;
      TH1D* hTempFail = new TH1D(histNameFail, "", nMassBin, minM_, maxM_ );
      vec_failHist_.push_back( hTempFail );
    }

    TString hEffTemplateName = "hEffTemplate"+binType_;
    if( tag_ != "" ) hEffTemplateName = hEffTemplateName + "_" + tag_;
    hEffTemplate_ = new TH1D(hEffTemplateName, "", nBin_, arr_binEdge_);
  }
...
public:
  ...
  TnPHistProducer( TString tag, Double_t ptCut )
  {
    tag_ = tag;
    Set_ptCut( ptCut );
    Init();
...
public:
  ...
  TGraphAsymmErrors* CalcTnPEff_CutAndCount( TString tag, TString varName )
  {
    TString hEffTemplateName = "hEffTemplate"+varName;
    if( tag != "" ) hEffTemplateName = hEffTemplateName + "_" + tag;
    TH1D* hEffTemp = PlotTool::Get_Hist( inputFileName_, hEffTemplateName, "hEff"+varName);
