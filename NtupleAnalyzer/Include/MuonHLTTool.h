...
Bool_t dRMatching_L2Muon( TLorentzVector vecP_ref, NtupleHandle* ntuple, Double_t minPt, Double_t minDR )
{
  ...
  vector<TLorentzVector> vec_vecP_L2Muon;
  for(const auto& L2Muon : vec_L2Muon )
    vec_vecP_L2Muon.push_back( L2Muon.vecP );

  return MuonHLT::dRMatching( vecP_ref, vec_vecP_L2Muon, minDR );
}
