...
def customizeForMuonHLTIsolation_differentRhoForECALHCAL(process):
    ...
    # -- sequence in MC_IsoTkMu15
    if hasattr(process, "HLTHighPtTkMuEcalPFisorecoSequenceNoBoolsForMuons"):
        process.HLTHighPtTkMuEcalPFisorecoSequenceNoBoolsForMuons = cms.Sequence( 
          process.HLTDoFullUnpackingEgammaEcalMFSequence + 
          process.HLTDoLocalHcalSequence + 
          process.hltFixedGridRhoFastjetECALMFForMuons + 
          process.hltFixedGridRhoFastjetHCAL + 
          process.HLTPFClusteringEcalMFForTkMuons + 
          process.hltHighPtTkMuonEcalMFPFClusterIsoForMuons )

    # -- sequence in DST_Run3_PFScoutingPixelTracking_v16
    if hasattr(process, "HLTL3muonEcalPFisorecoSequenceNoBoolsForMuonsNoVtx"):
        process.HLTL3muonEcalPFisorecoSequenceNoBoolsForMuonsNoVtx = cms.Sequence( 
          process.HLTDoFullUnpackingEgammaEcalMFSequence + 
          process.HLTDoLocalHcalSequence + 
          process.hltFixedGridRhoFastjetECALMFForMuons + 
          process.hltFixedGridRhoFastjetHCAL + 
          process.HLTPFClusteringEcalMFForMuonsNoVtx + 
          process.hltMuonEcalMFPFClusterIsoForMuonsNoVtx )

    return process

# -- update the WP for the tracker isolation: from 0.07 to 0.08
# -- name of the modules are also updated properly
def customizerForMuonHLT_newTrkIsoWP(process):
        from HLTrigger.Configuration.common import insert_modules_after, filters_by_type

        if hasattr(process, "hltMuonTkRelIsolationCut0p07Map"):
            process.hltMuonTkRelIsolationCut0p08Map = process.hltMuonTkRelIsolationCut0p07Map.clone()
            process.hltMuonTkRelIsolationCut0p08Map.CutsPSet.Thresholds = cms.vdouble( 0.08 )

            insert_modules_after(process, process.hltMuonTkRelIsolationCut0p07Map, process.hltMuonTkRelIsolationCut0p08Map)
            ...
            if hasattr(module, "DepTag"):
                ...
                for index in range(0, len(_vinputtag)):
                    if _vinputtag[index] == "hltMuonTkRelIsolationCut0p07Map":
                        _vinputtag[index] = "hltMuonTkRelIsolationCut0p08Map"
                        isFound = True
        ...
        for module in dic_module_before_after.keys():
            ...
            label_new = module._Labelable__label.replace("0p07", "0p08")
        ...
        for module in filters_by_type(process,"HLT2MuonPFTau"):
            if "Filtered0p07" in module.inputTag1.value():
                module.inputTag1 = module.inputTag1.value().replace("0p07","0p08")

        for module in filters_by_type(process,"HLTMuonL3PreFilter"):
            if "Filtered0p07" in module.PreviousCandTag.value():
                module.PreviousCandTag = module.PreviousCandTag.value().replace("0p07","0p08")
